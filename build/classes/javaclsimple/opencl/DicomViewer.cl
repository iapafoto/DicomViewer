...
inline float mixLin3(const float3 p, global read_only float* buf, const int3 sz) {
    const int w = sz.x*sz.y; 
    const float z = clamp(p.z*H_COEFF,0.f,(float)(sz.z)-1.f),
                kz = z-floor(z);
    buf += w*(int)(z);
    const float a0 = mixLin2(p.xy,buf,sz.xy);
    if (kz == 0.f) return a0;
    return mix(a0, mixLin2(p.xy,buf+w,sz.xy), kz); 
}
...
*/
float3 normalAt(const float3 p, global read_only float* buf, const int3 sz) {
    return normalize(ep.xyy * valueAt(p + ep.xyy, buf, sz)
                   + ep.yyx * valueAt(p + ep.yyx, buf, sz)
                   + ep.yxy * valueAt(p + ep.yxy, buf, sz)
                   + ep.xxx * valueAt(p + ep.xxx, buf, sz));
}
// -------------------------------------------------------------------
...
float preciseSurface(const float3 ro, const float3 rd, float val, float dmin, float dmax, float sig, const int dPrec, global read_only float* buf, const int3 sz) {
    float dm = dmin;
    val *= sig;  
    for (int j=0; j<dPrec;j++) {
       dm = (dmin + dmax)*.5f;  // on détermine l'indice de milieu
       if (valueAt(ro+rd*dm, buf, sz)*sig > val) dmax = dm;  //si la valeur qui est à la case "im" est supérieure à la valeur recherchée, l'indice de fin "ifin" << devient >> l'indice de milieu
...
float findPosOfVal(const float3 ro, const float3 rd, const float val, const float dd, const float dmin, const float dmax, const int dPrec, global read_only float* buf, const int3 bufSz) {
    ...
    const float sig = valueAt(p, buf, bufSz) > val ? -1.f: 1.f;
    ...
    if (sig*(v-val) > 0.f && dPrec)
        return preciseSurface(ro, rd, val, d-dd, d, sig, dPrec, buf, bufSz);
    return d;
}
...
float4 doShading(const float3 rd, const float3 p, const float3 n, const float3 LIGHT, const float4 col, const float4 boundsMin, const float4 boundsMax, global read_only float* buf, const int3 sz, const Quality* quality) { 
    ...
    const float ambiant = .6f;
    float3 c = (.65f+(.35f*ao)) * (col.xyz*(ambiant + diffuse*.4f) + specular*.3f) + .2f*rimCol;	 
    ...
    if (box(ro-center, rd, Bounds, &dmin, &dmax, &n)) {
       ...
       // coupe du rectangle externe
        ...
        if (val > bbMin.w && val <= bbMax.w) {
            col = doShading(rd,p,n,lightDir, col, bbMin, bbMax, buf, bufSz, quality);
        } else {    
...
#endif
   // write_imagef(outputImage, (int2)(x,y), (float4)(col.x,col.y,col.z,1.f));
}
...
__kernel void render(const float3 roo, const float3 ta, const float4 sliderMin, const float4 sliderMax, global read_only float* buf, const int3 bufSz, write_only image2d_t outputImage, const float2 deltaPix, const int qual) {
    ...
    const float4 bbMin = mix((float4)(2.f,2.f,3.f/H_COEFF,-1000), (float4)(bufSz.x-3, bufSz.y-3,(float)(bufSz.z-3.f)/H_COEFF,1000), sliderMin),
                 bbMax = mix((float4)(2.f,2.f,3.f/H_COEFF,-1000), (float4)(bufSz.x-3, bufSz.y-3,(float)(bufSz.z-3.f)/H_COEFF,1000), sliderMax);
    ...
    const Quality quality = (qual > 0 ? hq : lq);
    
    float s = 1.f;  
    const float3 ro = roo - ta + center;
    float3 rd;

    float4 colSum = (float4)(0.f);
    
    const float dAA = qual > 1 ? 1.f/4.f : 1.f;
    
    for (float i=0.f; i<1.f; i+=dAA) {
        for (float j=0.f; j<1.f; j+=dAA) {
            rd = RD(ro, center, (float)(x)+i, (float)(y)+j, outSize, 3.5f);
            colSum += renderRay(ro, rd, sliderMin, sliderMax, buf, bufSz, cback, &quality);   
        }
    }
    
    float4 col = clamp(colSum*dAA*dAA, (float4)(0.f), (float4)(1.f));
    
  //  write_imagef(outputImage, (int2)(x,y), (float4)(col.x,col.y,col.z,1.f));

    float4 out = (float4)(col,1.f);
#ifdef TEXTURE_OUT_FLOAT
    write_imagef(outputImage, (int2)(x, y), out); 
#else
    uint4 rgba = (uint4)((int)(out.x*256.f),
                         (int)(out.y*256.f),
                         (int)(out.z*256.f), 255);
    write_imageui(outputImage, (int2)(x, y), rgba); 
#endif
