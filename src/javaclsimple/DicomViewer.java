 ...
 * @author durands
 */
public class DicomViewer extends CLEditor {
...
//    private final static float[] demBuff = DataLoader.getDicom("C:\\dicom\\Cerveau", 0,50,80,510,450,350,demSize, minMax);
...
//    private final static float[] demBuff = DataLoader.getDicom("C:\\dicom\\Coeur", demSize, minMax);
    ...
    public DicomViewer() {
        super("DicomViewer.cl");
        
        clManager.createInput(PARAM_BUFFER_3D, demBuff);
    ...
    protected void updateKernelArgsAfterCompil() {
        ...
        clManager.setFloatBufferOnArg(PARAM_BUFFER_3D);     
    ...
    public void updateKernelArgsRenderingMode(boolean antialiasing) {
        final float 
                subX = (antialiasing) ? (float)Math.random() : 0.f,
                subY = (antialiasing) ? (float)Math.random() : 0.f;
   //     clManager.setImageOutOnArg(PARAM_IMG_OUT, antialiasing);
